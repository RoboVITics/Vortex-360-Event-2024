...
const Timeline = ({ imgSources, dateTimes }) => {
  const [currentImgIndex, setCurrentImgIndex] = useState(0);

  useEffect(() => {
    const intervalId = setInterval(() => {
      // Increment the current image index
      setCurrentImgIndex((prevIndex) => (prevIndex + 1) % imgSources.length);
    }, getTimeDifference(dateTimes[currentImgIndex]));

    // Clear the interval when the component is unmounted
    return () => clearInterval(intervalId);
  }, [currentImgIndex, imgSources, dateTimes]);

  const getTimeDifference = (targetDateTimeString) => {
  ...
  return (
    <div>
      <img src={imgSources[currentImgIndex]} alt={`img-${currentImgIndex}`} />
    </div>
  );
};
